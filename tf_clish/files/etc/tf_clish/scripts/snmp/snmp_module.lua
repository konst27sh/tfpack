...
function snmp_module.get_config()
    ...
    for _, v in pairs(snmp_config) do
        ...
        elseif v[".type"] == "snmp_v1" or v[".type"] == "snmp_v2c" then
            ...
            if v["read_community"] ~= nil and v["read_community"] ~= "" then
                v1v2_param["read_community"] = v["read_community"]
            else
                v1v2_param["read_community"] = "------"
            end

            if v["write_community"] ~= nil and v["write_community"] ~= "" then
                v1v2_param["write_community"] = v["write_community"]
            else
                v1v2_param["write_community"] = "------"
            end

            if v["read_access"] ~= nil and v["read_access"] ~= "" then
                v1v2_param["read_access"] = v["read_access"]
            else
                v1v2_param["read_access"] = "------"
            end

            if v["write_access"] ~= nil and v["write_access"] ~= "" then
                v1v2_param["write_access"] = v["write_access"]
            else
                v1v2_param["write_access"] = "------"
            end
