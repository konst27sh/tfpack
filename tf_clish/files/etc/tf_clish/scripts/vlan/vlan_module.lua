...
function vlan_module.showVlanInfo(vlan_arr, id)
    ...
    elseif (tonumber(id) ~= 0) then
        if vlan_arr[id] ~= nil then
            ...
            print("warning: Vlan " .. id .. " does not exist, choose *network vlan show status all* to inspect existing Vlan")
        end
    end
end
...
function vlan_module.getMngmntVlan(cmd, vlanArr, if_name)
    ...
    if devInfo ~= nil then
        ...
        if mngtVlan[1] == "switch" then
            local vlan_id = mngtVlan[2]
            if vlanArr[vlan_id] == nil then
                print("error: mngt_VLAN* " .. if_name .. "* without VLAN_ID")
            else
                vlanArr[vlan_id].mngtVlan = "yes"
                if vlanArr[vlan_id].ifname ~= nil then
                    ifname_table = vlanArr[vlan_id].ifname
                end
                table.insert(ifname_table, if_name)
                vlanArr[vlan_id].ifname = ifname_table
            end
        end
    else
...
local function set_portStatus(port_num, state, vid)
    print("info: port *" .. port_num .. ":" .. state .. "* is present in VLAN *" .. vid .. "*")
    --status = "busy"
    return "busy"
end

function vlan_module.add_untagged(vlanArr, vlan_id, portRange, action)
    local res_setVlan = 1
    ...
    if vlan_list == nil then
        print("warrning: vlan  *" .. vlan_index .. "* not presented")
        -- TODO create function to add new Vlan
        return res_setVlan
    else
        local busyList = {}
        local err_saveVlan = false
        busyList, err_saveVlan = vlan_utils.saveVlan(vlan_list, action)
        if not err_saveVlan then
            return res_setVlan
        end
        table.insert(busy_vlanPortList, vlan_index, busyList)
        ...
        for id, vlanList in pairs(vlanArr) do
            if vlanList.ports ~= nil then
                ...
                for portNum, state in pairs(vlanList.ports) do
                    --if i == port_num then
                    --    print("busy")
                    --    status = set_portStatus(port_num, state, vlanList.num)
                    --end
                    if action == "add" then
                        if i == portNum and state == "U" then
                            status = set_portStatus(portNum, state, vlanList.num)
                        elseif i == portNum and state == "T" then
                            if tonumber(id) < vlan_id or tonumber(id) > vlan_id then
                                status = set_portStatus(portNum, state, vlanList.num)
                            end
                        end
        ...
        if status == "free" then
            if busy_vlanPortList == nil then
                if action == "add" then
                    ...
                    table.insert(busy_vlanPortList, vlan_id, portList)
                elseif action == "edit" then
                    print("info: port " .. i .. " not set, first use the add command")
                end
            else
                for _, portList in pairs(busy_vlanPortList) do
                    if action == "add" then
    ...
    if add_count > 0 then
        ...
        res_setVlan = vlan_utils.uciSetVlanParam(vlanArr, busy_vlanPortList)
    else
        res_setVlan = "not_found"
    end
    return res_setVlan
end

function vlan_module.add_tagged(vlanArr, vlanRange, portRange, action)
    local res_setVlan = "error"
    local print_str = ""
    local portRangeMin = tonumber(portRange[1])
    local portRangeMax = tonumber(portRange[2])
    ...
    for i = vlanRangeMin, vlanRangeMax do
        ...
        vlan_list = vlanArr[vlan_index]
        if vlan_list == nil then
            print("warning: vlan  *" .. vlan_index .. "* not presented")
            return res_setVlan
            -- TODO create function to add new Vlan
        else
            local busyList = {}
            ...
            busyList, err_saveVlan = vlan_utils.saveVlan(vlan_list, action)
            if not err_saveVlan then
                print("err_saveVlan = " .. err_saveVlan)
                return res_setVlan
            end
            table.insert(busy_vlanPortList, vlan_index, busyList)
        end
            ...
            if vlanList.ports == nil then
                ...
                for portNum, state in pairs(vlanList.ports) do
                    if action == "add" then
                        ...
                        if i == portNum and state == "T" then
                            if tonumber(id) >= vlanRangeMin and tonumber(id) <= vlanRangeMax then
                                status = set_portStatus(portNum, state, vlanList.num)
                            end
                        end
                    elseif action == "edit" then
                        ...
                        elseif tonumber(id) >= vlanRangeMin and tonumber(id) <= vlanRangeMax then
                            if i == portNum and state == "T" then
                                status = set_portStatus(portNum, state, vlanList.num)
                            end
                        end
        ...
        if status == "free" then
            for vlan_id, portList in pairs(busy_vlanPortList) do
                if portList == nil then
                    print("error: port list is empty " .. " vlan_id = " .. vlan_id)
                end
                if action == "add" then
                    if portList[i] == nil then
                        portList[i] = "T"
...
function vlan_module.add_notMemberPorts(vlanArr, vlanRange, portRange)
    ...
    for i = vlanRangeMin, vlanRangeMax do
        ...
        if vlan_list == nil then
            ...
            local busyList = {}
            local err_saveVlan = false
            busyList, err_saveVlan = vlan_utils.saveVlan(vlan_list, "edit")
            if not err_saveVlan then
                return res_setVlan
            end
            table.insert(busy_vlanPortList, vlan_index, busyList)
...
function vlan_module.uci_addVlan(id)
    local value = ""
    value = "vlan" .. id
    local bridge_id_cmd = "ubus call uci add '{\"config\":\"network\",\"type\":\"bridge-vlan\", \"name\":\"" .. value .. "\"}'"
    --print("bridge_id = " .. bridge_id)
    tf.executeCommand(bridge_id_cmd)
...
local function checkVlan_is_valid(vlanArr, vlan_id, if_name, if_deleted_list)
    ...
    local ifList, iflist_res = br.br_getIflist(vlan_module.cmd_vlan_list.cmd_netIfInfo)
    if iflist_res ~= 0 then
        print(pm.critical.if_list)
        return status_vid, status_ifname
    end
    if #ifList == 1 then
        status_ifname = "can_not_delete"
    else
        for i, ifname in pairs(ifList) do
...
function vlan_module.mngtVlan_handler(vlanArr, vlanID, if_name, action, if_deleted_list)
    if #if_name > 16 then
        print("error: The name is too long, max 16 characters")
        return 2
    end
    local status_vid, status_ifname = checkVlan_is_valid(vlanArr, vlanID, if_name, if_deleted_list)
    --print("status_vid = " .. status_vid .. " status_ifname = " .. status_ifname)
    if action == "add" then
        if status_vid == "not_found" or status_ifname == "if_name_busy" or status_ifname == "if_name_deleted" then
            if status_vid == "not_found" then
                print(pm.warning.vlan_not_found .. " vlan id: " .. vlanID)
                return 2
            end
            if status_ifname == "if_name_busy" then
                print(pm.warning.ifname_used .. " ifname: " .. if_name)
                return 2
            end
            if status_vid == "if_name_deleted" then
                print(pm.warning.ifname_deleted .. " ifname: " .. if_name)
                return 2
            end
        else
            local config = "\"config\":\"network\""
            local section = "\"section\":\"" .. if_name .. "\""
            local section_type = "\"type\":\"interface\""
            local name = "\"name\":\"" .. if_name .. "\""
            local values = "\"values\":{\"device\":\"" .. dev_name .. "." .. vlanID .. "\", \"tfdm\":\"1\"}"
            local cmd_add = "ubus call uci add '{" .. config .. ", " .. section .. ", " .. section_type .. ", " .. name .. "}'"
            local cmd_set = "ubus call uci set '{" .. config .. ", " .. section .. ", " .. section_type .. ", " .. values .. "}'"
            local res = tf.executeCommand(cmd_add)
            if res ~= nil and res ~= "" then
                res = tf.decodeToJson(res)
            end
            if res.section ~= nil and res.section == if_name then
                tf.executeCommand(cmd_set)
                print(pm.info.if_config)
                return 0
            end
            return 2
        end
    elseif action == "edit" then
        if status_vid == "not_found" or status_ifname == "not_found" or status_ifname == "if_name_deleted" then
            if status_vid == "not_found" then
                print("warning: vlan id *" .. vlanID .. "* not found")
                return 2
            end
            if status_ifname == "not_found" then
                print("warning: interface *" .. if_name .. "* not found")
                return 2
            end
            if status_ifname == "if_name_deleted" then
                print("warning: interface *" .. if_name .. "* marked as DELETED. Needs to save or revert changes")
                return 2
            end
        else
            local config = "\"config\":\"network\""
            ...
            local cmd_set = "ubus -S call uci set '{" .. config .. ", " .. section .. ", " .. type .. ", " .. values .. "}'"
            tf.executeCommand(cmd_set)
            return 0
        end
    elseif action == "delete" then
        if status_ifname == "can_not_delete" then
            print("warning: This interface *" .. if_name .. "* is only one and cannot be deleted. First, add another one interface")
            return 2
        elseif status_ifname == "not_found" or status_ifname == "if_name_deleted" then
            if status_ifname == "not_found" then
                print("warning: interface *" .. if_name .. "* not found")
            elseif status_ifname == "if_name_deleted" then
                print("warning: interface *" .. if_name .. "* marked as DELETED. Needs to save or revert changes")
            end
            return 2
        else
