...
function tf_module.getUbusDataByName(name)
    local ubus_cmd = "ubus call tf_hwsys getParam '{\"name\":\"" .. name .. "\"}'"
    local jsonInfo = getJsonTable(ubus_cmd)
    return jsonInfo
end

function tf_module.executeCommand(command, isPrint)
...
function tf_module.collectJsonTable(ubusCmd)
    if ubusCmd ~= nil and ubusCmd ~= "" then
...
function tf_module.saveChanges(isPrint, demon)
    ...
    if isPrint == 1 then
        print("info: new configuration saved")
    end
...
function tf_module.isValidIP(ip)
...
function tf_module.parse_ip(ipString)
    ...
    for ip in ipString:gmatch("[^,]+") do
        ip = removeSpaces(ip)
        if tf_module.isValidIP(ip) then
...
function tf_module.splitString(inputString)
    local words = {}
...
function tf_module.isNotFilePath(input)
    local isPath = input:find("[\\/:%*%?\"<>|]")
    if isPath then
        return false
    else
        return true
