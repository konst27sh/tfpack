...
function formatPoe(poe) {

	var e = E('span');
	if (poe) {
		if(poe.status == "Delivering power") {
			e.innerText = 'PoE '+parseFloat(poe.power).toFixed(1)+'W';
		}
	}
...
return baseclass.extend({
	...
	load: function() {
		return Promise.all([
			...
			network.getNetworks(),
			uci.load('network'),
			L.resolveDefault(callPoeStatus(), [])
		]);
	},
	...
	render: function(data) {
		...
		return E('div', { 'style': 'display:grid;grid-template-columns:repeat(auto-fit, minmax(60px, 1fr));margin-bottom:1em' }, known_ports.map(function(port) {
			var speed = port.netdev.getSpeed(),
				...
				poe = data[5].ports[port.netdev.getName()];


			return E('div', { 'class': 'ifacebox', 'style': 'margin:.25em;min-width:60px;max-width:100px' }, [
				E('div', { 'class': 'ifacebox-head', 'style': 'font-weight:bold' }, [ getPortName(port.netdev.getName()) ]),
