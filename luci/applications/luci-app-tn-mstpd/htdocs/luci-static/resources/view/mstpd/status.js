	...
	renderWidget: function(section_id, option_id, cfgvalue) {
		...
		if (!br || !br.hasOwnProperty('bridge')) {
			return E('div', { 'class': 'alert-message warning' },
				_('No data to display.<br>STP/RSTP is disabled')
			);
		}
		...
		items.push(mstpd_br_param_add(br, 'last-topology-change-port', function(br, v) {
			return ((v == 'None') || (v == '')) ? _('none', 'Port name') : getPortName(v)
		}));
		...
		br.ports.forEach(function(p) {
			...
			rows.push([
				...
				E('div', { 'style': 'display: flex; flex-flow: row;' }, [
					...
					E('span', {}, getPortName(mstpd_br_port_param_add(br, p, 'port')))
				]),
...
return L.view.extend({
	...
	renderData: function(bridges) {
		...
		m = new form.Map('mstpd');

		m.tabbed = (bridges.length > 1) ? true : false;

		if (bridges.length == 0) {
			s = m.section(form.NamedSection, 'global', 'mstpd',_('STP/RSTP Status'),
			...
			o.render = function() {
				return E('div', { 'class': 'alert-message warning' },
					_('No data to display. You must enable STP/RSTP service first')
						.format(L.url('admin/mstpd/config'))
				);
			};
