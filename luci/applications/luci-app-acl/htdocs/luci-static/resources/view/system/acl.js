...
function generateHash(section_id,user, pass, level){
...
return view.extend({
	...
	render: function(data) {
		...
		o.write = function(section_id, value) {
			...
			if(typeof this.section.formvalue(section_id, '_level') !== "undefined"){
				if(this.section.formvalue(section_id, '_level') == "write"){
					...
					generateHash(section_id,this.section.formvalue(section_id, 'username'),value,'full');
		...
		o.modalonly = true;
		o.default = "no";
		o.value('no', _('---', 'Please select permissions'));
		...
		o.cfgvalue = function(section_id) {
			...
			if (writeList.length == 1 && writeList[0] == '*')
				...
				return 'read';
			else if(writeList.length!=0 || readList.length!=0)
				return 'individual';
		};
		...
		o.validate = function(section_id, value) {
			if(value == 'no'){
				return _('Access level mismatch');
			}
			return true;
		};
		o.remove = function() {};
