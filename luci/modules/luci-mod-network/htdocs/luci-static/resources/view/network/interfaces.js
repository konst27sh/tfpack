...
function render_status(node, ifc, with_device) {
    let desc = null, c = [];
    const dev = ifc ? (ifc.getDevice() || ifc.getL3Device() || ifc.getL3Device()) : null;
    const vlan = dev ? dev.device : null;
    const vlan_id = vlan ? vlan.replace('switch.', '') : null;
...
return view.extend({
    ...
    render: function(data) {
        ...
        s = m.section(form.GridSection, 'interface', _('Interfaces'),_('List of managment interfaces'));
        s.anonymous = true;
        s.addremove = true;
        ...
        s.handleAdd = function(ev) {
            ...
            proto.value("dhcpv6","DHCPv6-Client");
            ...
            device.vlanname = true;

            device.filter = function(section_id, name) {
                var dev = this.devices.filter(function(dev) {return dev.getName() == name })[0];
                return (dev && dev.getType() == 'vlan');
            };
        ...
        o = s.taboption('advanced', form.Flag, 'tfdm', _('TFortis Device Manager agent'), _('You san enable or disable searching through TFortis Device Manager on selected interfaces'));
        o.modalonly = true;
        o.defaults = o.enabled;
