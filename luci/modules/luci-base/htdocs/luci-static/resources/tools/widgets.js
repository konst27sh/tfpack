...
var CBIDeviceSelect = form.ListValue.extend({
	...
	renderWidget: function(section_id, option_index, cfgvalue) {
		...
		for (var i = 0; i < this.devices.length; i++) {
			...
			if (this.vlanname){
				devname = device.getTypeI18n() + ' ' +  device.config.vid;
			}
			else{
				devname = device.getI18n();
			}
			...
			var item = E([
				...
				E('span', { 'class': 'hide-open' }, [ devname ]),
				E('span', { 'class': 'hide-close'}, [ devname ])
			]);
		...
		if (this.networks != null) {
			for (var i = 0; i < this.networks.length; i++) {
				...
				var item = E([
					...
					E('span', { 'class': 'hide-close'}, [ devname/*device.getI18n()*/ ])
				]);
