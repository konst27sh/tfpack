...
function validateIP(section_id, value) {
    ...
    console.log("Validate "+section_id+" "+value);


    if(typeof  networks_validate !== 'undefined'){
        var form_validate = calculateNetwork(value,maskval)
        if(form_validate[0] == networks_validate[0])
            return _("IP address from this subnet is already exist on another management interface");
    }
    ...
    L.resolveDefault(network.getNetworks()).then(function(data) {
        for (var i = 1; i < data.length; i++){
            var addr = data[i].getIPAddr().toString();
            var mask = data[i].getNetmask().toString();
            if(data[i].sid != section_id){;
                networks_validate = calculateNetwork(addr,mask);
            }
        }
    });
...
return network.registerProtocol('static', {
    CBIIPValue: form.Value.extend({
        ...
        renderWidget: function(section_id, option_index, cfgvalue) {
            ...
            //отключение возможности создания множества IP на одном интерфейсе
            /*if (widget == 'Value')
...
**/
        ...
        validate: validateIP/*validateBroadcast*/
    }),
