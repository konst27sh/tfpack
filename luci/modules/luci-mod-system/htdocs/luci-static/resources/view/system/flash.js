...
return view.extend({
	...
	handleRecovery: function(ev) {
		if (!confirm(_('Do you really want to load Recovery?')))
			return;

		ui.showModal(_('Rebooting...'), [
			E('p', { 'class': 'spinning' }, _('The system is currently booting into recovery mode with default settings.<br>Access is only possible at http://192.168.1.1 '))
		]);

		/* Currently the sysupgrade rpc call will not return, hence no promise handling */
		fs.exec('/sbin/recoveryboot');
	...
	render: function(rpc_replies) {
		...
		if (has_rootfs_data) {
			...
			o = ss.option(form.Button, 'recovery', _('Reboot into recovery mode'),
				_('The system will be rebooted in recovery mode with the default management IP address'));
			o.inputstyle = 'negative important';
			o.inputtitle = _('Load recovery');
			o.onclick = this.handleRecovery;
		}
