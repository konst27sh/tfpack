...
void getPortParam(uint8_t index, uint8_t isReboot)
{
    ...
    getValueFromJson(res, "mode", value_state_str);
    ...
    if (typeOfTest >= test_max)
    {
        ...
        syslog(LOG_ERR, "port <%d>: error: test type", port);
        closelog();
    }
...
static error_code_t setHost(port_config_info_t *portConfig, char *jsonData)
{
    ...
    if (res ^ 8) {
        ...
        openlog("tf_autoresart", 0, LOG_USER);
        syslog(LOG_ERR, "port <%d>: test disabled - IP not valid", portConfig->portNum);
        closelog();
    }
    else
    {
...
static error_code_t setSpeed(port_config_info_t *portConfig, char *jsonData)
{
    ...
    {
        openlog("tf_autoresart", 0, LOG_USER);
        syslog(LOG_ERR, "port <%d>: test disabled - speed value not valid", portConfig->portNum);
        closelog();
    }
    return errorCode;
}
...
static error_code_t setTime(port_config_info_t *portConfig, char *jsonData)
{
    ...
    {
        openlog("tf_autoresart", 0, LOG_USER);
        syslog(LOG_ERR, "port <%d>: Alarm disabled - time not valid", portConfig->portNum);
        closelog();
